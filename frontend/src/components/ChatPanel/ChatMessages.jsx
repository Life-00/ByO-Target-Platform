    ...
    >
      ...
      {msg.visualizations && Object.keys(msg.visualizations).length > 0 && (
        <div className="mt-4 space-y-4 border-t pt-4">
          <h4 className="font-semibold text-sm mb-2">📊 시각화</h4>
          {msg.visualizations.evidence_network && (
            <div className="border rounded p-2 bg-white">
              <p className="text-xs font-medium mb-2">증거 네트워크</p>
              <iframe
                srcDoc={msg.visualizations.evidence_network}
                className="w-full h-96 border-0"
                title="Evidence Network"
              />
            </div>
          )}
          {msg.visualizations.feasibility_chart && (
            <div className="border rounded p-2 bg-white">
              <p className="text-xs font-medium mb-2">타당성 평가</p>
              <iframe
                srcDoc={msg.visualizations.feasibility_chart}
                className="w-full h-80 border-0"
                title="Feasibility Chart"
              />
            </div>
          )}
          {msg.visualizations.comparison_chart && (
            <div className="border rounded p-2 bg-white">
              <p className="text-xs font-medium mb-2">비교 분석</p>
              <iframe
                srcDoc={msg.visualizations.comparison_chart}
                className="w-full h-80 border-0"
      ...
      {msg.usage && (
        <div className="text-[10px] mt-2 opacity-60 border-t border-white/10 pt-1">
          토큰: {msg.usage.total_tokens} (비용: $
          {msg.usage.estimated_cost_usd?.toFixed(6) || "0.000000"})
        </div>
      )}
      <span
        className={`text-[10px] block mt-1 opacity-60 ${isUser ? "text-gray-300" : "text-gray-400"}`}
      >
...
const TypingIndicator = memo(({ agentMode }) => (
  <div className="flex gap-3">
    <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center flex-shrink-0 border border-gray-200">
      ...
      {agentMode === "report" && (
        <PenTool size={14} className="text-orange-600" />
      )}
    ...
    <div className="bg-white border border-gray-200 rounded-2xl rounded-tl-none p-4 shadow-sm flex items-center gap-1">
      <span
        className={`w-2 h-2 rounded-full animate-bounce ${agentMode === "analysis" ? "bg-purple-400" : agentMode === "report" ? "bg-orange-400" : agentMode === "search" ? "bg-blue-400" : "bg-teal-400"}`}
      ></span>
      <span
        className={`w-2 h-2 rounded-full animate-bounce delay-75 ${agentMode === "analysis" ? "bg-purple-400" : agentMode === "report" ? "bg-orange-400" : agentMode === "search" ? "bg-blue-400" : "bg-teal-400"}`}
      ></span>
      <span
        className={`w-2 h-2 rounded-full animate-bounce delay-150 ${agentMode === "analysis" ? "bg-purple-400" : agentMode === "report" ? "bg-orange-400" : agentMode === "search" ? "bg-blue-400" : "bg-teal-400"}`}
      ></span>
    </div>
  </div>
));
