    ...
    <div className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 shadow-sm z-10">
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 truncate max-w-[200px] md:max-w-xs">
          {isReport ? (
            <span className="bg-indigo-100 text-indigo-700 text-[10px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider shrink-0">Report</span>
          ) : (
            <span className="bg-teal-100 text-teal-700 text-[10px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider shrink-0">Paper</span>
          )}
          <h2 className="font-semibold text-gray-800 text-sm truncate" title={paper.title}>
            {paper.title}
          </h2>
        </div>
      </div>
        ...
        {viewMode === 'summary' && (
          <>
            ...
            <div className="flex items-center bg-gray-100 rounded-lg p-1">
              ...
              <span className="text-xs font-mono w-12 text-center text-gray-600">{zoomLevel}%</span>
              <button
                onClick={onZoomIn}
                className="p-1 hover:bg-white rounded shadow-sm text-gray-600"
              >
